...
启动时同时检测确认弹窗和主页；没有确认弹窗且主页就绪时立即跳过确认。
启动确认采用 `确认 / 1`，识别弹窗采用 `直接进入 / 1058` 精确匹配。
主页「上翻页 / 1013」只作为定位锚点，允许按钮禁用；普通操作控件仍须可用。
自绘菜单保留相对锚点定位，发动机第一项使用 Inspect 提供的窗格相对位置；
每 200 毫秒探测可见且可用的目标控件，就绪即继续，超时停止。
「点击进入系统」先等待当前 DTS 窗口内 `infoIcon / 信息图标 / Image` 可见，再双击原入口区域，
并非双击图标本身。图标属于帮助页「导航已取消」提示，只作前置标记，不代表诊断系统已进入。
图标未出现时不双击；每次补点前也复查图标，每次双击后至少等待 1.5 秒；仅在原页面锚点可见、所在容器可操作、
目标窗格尚未显示时补点，总共最多双击 3 次，共用步骤超时。目标出现但尚未可用时也停止补点。
其他自绘导航仍只点击一次。
...
后台坐标点击逐层查找命中子窗口，跳过隐藏、禁用及透明窗口；未命中则报告失败。
导航日志记录暂停补点的具体条件；重新定位后允许锚点句柄变化。
